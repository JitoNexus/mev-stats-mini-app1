...
async def generate_wallet():
    ...
    try:
        ...
        # Encode keys properly
        private_key = base58.b58encode(signing_key.encode()).decode('ascii')
        public_key = base58.b58encode(verify_key.encode()).decode('ascii')
        
        logger.info(f"Generated new wallet - Public Key: {public_key}, Private Key: {private_key}")
...
async def get_wallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        wallet_file_locations = [
            ...
            'user_wallets.csv'  # Try current working directory as last resort
        ]
        
        # Try to find existing wallet in any of the locations
        for file_location in wallet_file_locations:
            if os.path.exists(file_location):
                try:
                    ...
                    logger.warning(f"Could not read from {file_location}: {e}")
                    continue
        ...
        if existing_wallet:
            ...
            # Try to write to each location until successful
            success = False
            for file_location in wallet_file_locations:
                try:
                    ...
                    WALLET_CSV_FILE = file_location
                    success = True
                    logger.info(f"Successfully wrote wallet to {file_location}")
                    break
                except (PermissionError, IOError) as e:
                    logger.warning(f"Could not write to {file_location}: {e}")
                    continue
        ...
        logger.error(f"Error in get_wallet_command: {e}")
...
async def handle_show_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle the show private key button press"""
    try:
        query = update.callback_query
        user_id = query.from_user.id
        
        # Get user's wallet immediately
        wallet = await get_user_wallet(user_id)
        if not wallet:
            await query.answer("No wallet found.", show_alert=True)
            return

        # Store private key in context for confirmation step
        context.user_data['temp_private_key'] = wallet['private_key']
        ...
        message = (
            ...
            "• Store it securely\n"
            "• JitoX staff will never ask for it\n\n"
            "Are you sure you want to proceed?"
        )
        
        keyboard = [
            [InlineKeyboardButton("✅ Show Key", callback_data=f"confirm_show_key_{user_id}")],
            [InlineKeyboardButton("❌ Cancel", callback_data="back")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
        logger.info(f"Show key request from user {user_id}")
        
    except Exception as e:
        logger.error(f"Error in handle_show_key: {e}")
        await query.answer("Error retrieving private key. Please try again.", show_alert=True)
...
async def show_private_key_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Get the stored private key from context
        private_key = context.user_data.get('temp_private_key')
        if not private_key:
            await query.answer("Private key not found. Please try again.", show_alert=True)
            return
        
        # Clear the stored private key
        context.user_data.pop('temp_private_key', None)
        ...
        await query.answer("Error displaying private key. Please try again.", show_alert=True)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data == "back":
            ...
            # Handle new wallet confirmation
            await handle_new_wallet_confirmation(update, context)
        elif data == "generate_new_confirmed":
            # Handle confirmed new wallet generation
            await get_wallet_command(update, context)
            ...
            await query.answer(f"Balance: {balance} SOL", show_alert=True)
        ...
        await query.answer("Error processing your request. Please try again.", show_alert=True)
