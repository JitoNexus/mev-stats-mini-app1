...
async def recover_missed_users(application: Application) -> None:
    ...
    try:
        ...
        try:
            # Get recent messages from the admin chat
            messages = await application.bot.get_chat_history(chat_id=admin_chat_id, limit=1000)
            for message in messages:
                if message.from_user and str(message.from_user.id) not in existing_users:
                    missed_users.add((message.from_user.id, message.from_user.username))
...
def main() -> None:
    ...
    try:
        ...
        loop.run_until_complete(init_referral_db())
