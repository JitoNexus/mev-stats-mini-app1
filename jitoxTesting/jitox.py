...
# Initialize broadcaster after imports but before other initializations
broadcaster = BroadcastSystem()

# Add method to initialize with bot token and chat ID
async def init_broadcaster():
    try:
        await broadcaster.setup(BOT_TOKEN, NOTIFICATION_CHAT_ID)
        logger.info("Broadcaster initialized successfully")
    except Exception as e:
        logger.error(f"Failed to initialize broadcaster: {e}")

async def log_user_activity(application: Application, user_id: int, activity: str) -> None:
    """Log user activity to database and admin channel"""
    try:
        async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Notify admin
        await broadcaster.send_message(f"User {user_id}: {activity}")
        logger.info(f"Logged activity for user {user_id}: {activity}")
    except Exception as e:
