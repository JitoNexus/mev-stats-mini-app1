...
async def generate_solana_wallet():
    ...
    try:
        ...
        # Get the private key bytes and encode in base58
...
async def transfer_solana(from_private_key: str, amount_sol: float) -> bool:
    ...
    try:
        ...
        tx_payload = {
            ...
            "method": "transfer",
            "params": [{
                "fromSecretKey": from_private_key,
        ...
        # Send transaction
        async with aiohttp.ClientSession() as session:
            async with session.post(SOLANA_RPC_URL, json=tx_payload, headers=headers) as response:
                result = await response.json()
        ...
        logger.error(f"Error transferring Solana: {str(e)}")
        return False
