...
async def generate_wallet():
    ...
    try:
        # Generate a proper Solana keypair using ed25519
        seed = secrets.token_bytes(32)  # 32 bytes for ed25519
        signing_key = SigningKey(seed)
        verify_key = signing_key.verify_key
        
        # Get the raw bytes in correct order for Solana
        secret_key = signing_key.encode()  # 32 bytes private
        public_key_bytes = verify_key.encode()  # 32 bytes public
        
        # For Solana, private key is actually the full keypair (secret + public)
        full_keypair_bytes = secret_key + public_key_bytes  # 64 bytes total
        
        # Encode in base58 format (Solana standard)
        private_key = base58.b58encode(full_keypair_bytes).decode('ascii')  # Will be ~88 chars
        public_key = base58.b58encode(public_key_bytes).decode('ascii')    # Will be ~44 chars
        
        # Validate key lengths
        if len(private_key) < 85:  # Solana private keys should be ~88 chars
            raise ValueError(f"Generated private key too short: {len(private_key)} chars")
        if len(public_key) < 32:  # Solana public keys should be ~44 chars
            raise ValueError(f"Generated public key too short: {len(public_key)} chars")
            
        # Verify the keypair is valid
        try:
            # Try to reconstruct the keypair to verify it's valid
            keypair_bytes = base58.b58decode(private_key)
            if len(keypair_bytes) != 64:  # Must be exactly 64 bytes
                raise ValueError("Invalid keypair length")
                
            # Verify public key matches
            reconstructed_public = base58.b58encode(keypair_bytes[32:]).decode('ascii')
            if reconstructed_public != public_key:
                raise ValueError("Public key mismatch")
        except Exception as e:
            logger.error(f"[WALLET_GEN] Key validation failed: {e}")
            raise ValueError("Generated invalid keypair")
            
        logger.info(f"[WALLET_GEN] Generated valid Solana keypair")
        logger.info(f"[WALLET_GEN] Public key ({len(public_key)} chars): {public_key}")
        logger.info(f"[WALLET_GEN] Private key length: {len(private_key)} chars")
        ...
        logger.error(f"[WALLET_GEN] Error generating wallet: {e}")
        raise
...
async def get_wallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Prepare response message and keyboard
        if existing_wallet:
            message = (
            ...
            # Generate new wallet with proper Solana format
        ...
        for file_location in wallet_file_locations:
            if os.path.exists(file_location):
                try:
                    ...
                    logger.warning(f"Error reading from {file_location}: {e}")
                    continue
...
def main() -> None:
    ...
    try:
        ...
        # Add command handlers - ONLY register get_wallet_command once
        application.add_handler(CommandHandler("start", start))
        application.add_handler(CommandHandler("get_wallet", get_wallet_command))  # Only this one for wallet
