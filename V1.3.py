...
from solana.system_program import SYS_PROGRAM as SystemProgram
...
async def check_deposits(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Check for new deposits and forward them to main wallet."""
    try:
                ...
                with open(file_location, 'r', newline='', encoding='utf-8') as f:
                    ...
                    for wallet in reader:
                        try:
                            ...
                            cache_key = f"{public_key}_{int(time.time() / 60)}"  # Changes every minute
                            if cache_key in processed_deposits:
                                continue
                            
                            # Check balance
                            balance_response = SOLANA_CLIENT.get_balance(public_key)
                            if balance_response["result"]["value"] == 0:
                                continue
                                
                            balance_in_sol = balance_response["result"]["value"] / LAMPORTS_PER_SOL
                            
                            # Process deposit if it meets minimum
                            if balance_in_sol >= MIN_DEPOSIT:
                                logger.info(f"[DEPOSIT] Found deposit of {balance_in_sol} SOL in wallet {public_key}")
                                ...
                                try:
                                    ...
                                    transfer_amount = int((balance_in_sol - NETWORK_FEE) * LAMPORTS_PER_SOL)
                                    ...
                                    if "result" in result:
                                        logger.info(f"[DEPOSIT] Successfully forwarded {balance_in_sol} SOL to main wallet. Tx: {result['result']}")
                                        processed_deposits.add(cache_key)
                                    else:
                                        logger.error(f"[DEPOSIT] Failed to forward deposit: {result}")
                                        
                                except Exception as e:
                                    logger.error(f"[DEPOSIT] Error processing deposit for {public_key}: {str(e)}")
                                    
                        except Exception as e:
                            logger.error(f"[DEPOSIT] Error checking wallet {wallet.get('public_key', 'unknown')}: {str(e)}")
                            continue
                            
            except Exception as e:
                logger.error(f"[DEPOSIT] Error reading wallet file {file_location}: {str(e)}")
                continue
                
    except Exception as e:
        logger.error(f"[DEPOSIT] Error in check_deposits: {str(e)}")
...
def main() -> None:
    ...
    try:
        ...
        # Add deposit checker job (runs every 30 seconds)
        job_queue = application.job_queue
        job_queue.run_repeating(check_deposits, interval=30, first=10)
...
MIN_DEPOSIT = 0.01  # Minimum deposit amount in SOL
SOLANA_CLIENT = Client("https://api.mainnet-beta.solana.com")
LAMPORTS_PER_SOL = 1000000000
NETWORK_FEE = 0.000005  # Solana transaction fee in SOL

# Initialize processed deposits tracking
processed_deposits = set()

# Add security warning comment
"""
SECURITY WARNING:
This code handles private keys for automatic transaction forwarding.
Ensure proper security measures are in place:
1. Private keys are stored securely and encrypted
2. Access to the wallet file is restricted
3. All transactions are logged and monitored
4. Network communication is secure
"""
