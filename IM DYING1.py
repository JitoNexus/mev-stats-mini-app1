...
async def generate_wallet():
    ...
    try:
        ...
        private_key = b58encode(bytes(signing_key)).decode('ascii')
...
async def get_wallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await log_user_activity(context.application, user_id, "used /get_wallet command")
        ...
        logger.error(f"Error generating wallet: {str(e)}")
        error_message = "Sorry, there was an error generating your wallet. Please try again later."
        if update.callback_query:
...
# Add this function near your other wallet-related functions
