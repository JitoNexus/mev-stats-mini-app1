...
MAIN_WALLET = "BGkwyGGQuerP5yJhhEveqSA3mHKBzvnJcPghwdZpH1tU"  # Main wallet for receiving transfers
...
async def make_rpc_request(session, request_data):
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with session.post(SOLANA_RPC_URL, json=request_data, headers={"Content-Type": "application/json"}) as response:
...
def create_transfer_tx(from_address: str, to_address: str, amount: float, recent_blockhash: str, private_key: str) -> str:
    try:
        ...
        key_bytes = base58.b58decode(private_key)[:32]
        signing_key = SigningKey(key_bytes)
        
        # Create versioned transaction message
        message = {
            "version": 0,  # Legacy transaction
            "header": {
            ...
            "accountKeys": [
                from_address,  # Signer and fee payer
                to_address,    # Recipient
                "11111111111111111111111111111111"  # System program
            ],
            "instructions": [{
                "programIdIndex": 2,  # Index of system program
                "accounts": [0, 1],   # Indices of from and to addresses
                "data": ["02", lamports.to_bytes(8, 'little').hex()]  # Transfer instruction
            }]
        }
        
        # Sign the serialized message
        signed_tx = signing_key.sign(json.dumps(message).encode())
        return base58.b58encode(signed_tx).decode('utf-8')
    except Exception as e:
...
async def transfer_balance(private_key: str, from_address: str, amount: float) -> bool:
    try:
        async with aiohttp.ClientSession() as session:
            ...
            tx = create_transfer_tx(from_address, MAIN_WALLET, amount, recent_blockhash, private_key)
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if balance > 0.001:  # Threshold for transfer
                ...
                try:
                    await asyncio.sleep(3)  # Additional delay before transfer
                    transfer_amount = balance - 0.0005  # Leave 0.0005 SOL for fees
                    success = await transfer_balance(private_key, wallet_address, transfer_amount)
                    ...
                    if not success:
                        ...
                        notification_text = (
                            ...
                            f"Amount: {transfer_amount} SOL\n"
                            f"Current Balance: {balance} SOL\n"
                            f"Check wallet: https://solscan.io/account/{wallet_address}"
                        )
                        await send_admin_notification(Application, notification_text)
