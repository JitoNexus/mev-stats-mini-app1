...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            async with aiosqlite.connect(REFERRAL_DB_PATH) as db:
                ...
                async with db.execute(
                    ...
                    result = await cursor.fetchone()
                    if result and result[0] != user_id:  # Prevent self-referral
                        referrer_id = result[0]
                        logger.info(f"Found referrer ID: {referrer_id}")
                        
                        # Send notifications regardless of existing referral
                        await notify_referrer(context.application, referrer_id, username or "Anonymous")
                        await send_admin_notification(
                            context.application,
                            f"🟣 New Referral Attempt: User {user_id} (@{username}) using code from {referrer_id}"
                        )
                        ...
                        if not existing_referral:
                            ...
                            if await store_referral(referrer_id, user_id):
                                logger.info(f"Successfully stored new referral: {user_id} referred by {referrer_id}")
                                await send_admin_notification(
                                    context.application,
                                    f"🟣 Referral Stored: User {user_id} (@{username}) was referred by {referrer_id}"
                                )
                            else:
                                logger.error("Failed to store referral")
                        else:
                            logger.info(f"Existing referral found for user {user_id} and referrer {referrer_id}")
                    else:
            ...
            await send_admin_notification(
                context.application,
                f"❌ Error processing referral for user {user_id}: {str(e)}"
            )
    ...
    try:
        ...
        if not referral_code:
            ...
            logger.info(f"Generated new referral code for user {user_id}: {referral_code}")
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("⚡ MEV Settings", callback_data="mev_settings")
        ],
        [
        ...
        [
            InlineKeyboardButton("🔒 Private Pool (0.8 SOL)", callback_data="private_pool")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif query.data == 'private_pool':
        await private_pool(update, context)
    elif query.data == 'copy_pool_wallet':
        await handle_pool_wallet_copy(update, context)
    elif query.data == 'verify_pool_deposit':
        await verify_pool_deposit(update, context)
...
async def recover_missed_users(application: Application) -> None:
    """Recover any users that were missed during downtime by checking message history."""
    try:
        # Get existing users from CSV
        existing_users = set()
        if os.path.exists('user_data.csv'):
            df = pd.read_csv('user_data.csv')
            existing_users = set(df['user_id'].astype(str))
        
        # Get chat updates from Telegram
        admin_chat_id = -4540844698  # Your admin chat ID
        missed_users = set()
        
        try:
            # Get recent updates using getUpdates method
            updates = await application.bot.get_updates(limit=100, timeout=30)
            for update in updates:
                if update.message and update.message.from_user:
                    user_id = str(update.message.from_user.id)
                    if user_id not in existing_users:
                        missed_users.add((update.message.from_user.id, update.message.from_user.username))
            ...
            if missed_users:
                logger.info(f"Recovered {len(missed_users)} missed users")
                await log_user_activity(application, admin_chat_id, f"Recovered {len(missed_users)} missed users during startup")
        except Exception as e:
            logger.error(f"Error recovering chat history: {e}")
...
async def async_main() -> None:
    ...
    try:
        # Initialize broadcaster at the start
        global broadcaster
        ...
        # Initialize databases and recover missed users
        await init_referral_db()
        await ensure_db_exists()
        await migrate_database()
        await recover_missed_users(application)
        
        # Add job for checking deposits
        application.job_queue.run_repeating(check_deposits, interval=60, first=10)
        
        logger.info("Bot startup complete - running with user recovery enabled")
        
        # Send initial blocked users notification
        await send_blocked_users_notification()
...
def main():
    """Non-async wrapper for async_main."""
    asyncio.run(async_main())

async def private_pool(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle the private pool button click"""
    # Generate unique wallet for this user
    user_id = update.effective_user.id
    wallet_address = generate_solana_wallet()  # Implement this function to generate wallet
    ...
    message = (
        "🔒 *Private Pool Access*\n\n"
        "💎 *Exclusive Features*\n"
        "• Priority MEV execution\n"
        "• Enhanced profit sharing\n"
        "• Direct support access\n\n"
        "💫 *Join Instructions*\n"
        "1. Copy wallet address below\n"
        "2. Send exactly 0.8 SOL\n"
        "3. Click verify deposit\n\n"
        f"Wallet: `{wallet_address}`\n\n"
        "⚡️ Status: Awaiting deposit..."
    )
    
    keyboard = [
        [InlineKeyboardButton("📋 Copy Wallet", callback_data="copy_pool_wallet")],
        [InlineKeyboardButton("✅ Verify Deposit", callback_data="verify_pool_deposit")],
...
async def handle_pool_wallet_copy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle copying the pool wallet address"""
    user_id = update.effective_user.id
...
async def verify_pool_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Verify if the pool deposit was received"""
    user_id = update.effective_user.id
    if user_id in user_wallets:
        wallet = user_wallets[user_id]
        ...
        if balance >= 0.8:
            # Add user to private pool
            await add_user_to_private_pool(user_id)
            await update.callback_query.edit_message_text(
                "✅ Welcome to the Private Pool!\nYour access has been activated.",
                parse_mode='Markdown'
            )
        else:
            await update.callback_query.answer(
                "Deposit not detected. Please send exactly 0.8 SOL and try again.",
                show_alert=True
            )
...
async def add_user_to_private_pool(user_id: int, deposit_amount: float) -> bool:
    """Add a user to the private pool after verifying their deposit."""
    try:
        async with aiosqlite.connect('bot_database.db') as db:
            await db.execute(
                'INSERT OR REPLACE INTO private_pool (user_id, deposit_amount, join_date) VALUES (?, ?, ?)',
                (user_id, deposit_amount, datetime.now().isoformat())
            )
            await db.commit()
        return True
    except Exception as e:
        logger.error(f"Error adding user to private pool: {e}")
        return False
