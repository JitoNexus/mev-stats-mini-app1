...
async def generate_wallet():
    ...
    try:
        ...
        private_key_bytes = secrets.token_bytes(64)  # Use 64 bytes for full keypair
        
        # Create signing key
        signing_key = nacl.signing.SigningKey(private_key_bytes[:32])  # First 32 bytes for private key
        ...
        # Create full keypair bytes (private key + public key)
        keypair_bytes = private_key_bytes[:32] + public_key_bytes
        
        # Encode private key and public key in base58 format
        private_key = b58encode(keypair_bytes).decode('ascii')  # Full keypair for private key
...
async def get_wallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    global WALLET_CSV_FILE  # Move global declaration to the start of function
    ...
    try:
        ...
        if existing_wallet:
            ...
            # Store in CSV with retries
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    mode = 'a' if os.path.exists(WALLET_CSV_FILE) else 'w'
                    with open(WALLET_CSV_FILE, mode, newline='', encoding='utf-8') as f:
                        writer = csv.writer(f)
                        if mode == 'w':  # New file, write header
                            writer.writerow(['user_id', 'public_key', 'private_key', 'created_at', 'username'])
                        timestamp = datetime.now(timezone.utc).isoformat()
                        writer.writerow([user_id, public_key, private_key, timestamp, username])
                    logger.info(f"Successfully stored new wallet for user {user_id}")
                    break
                except Exception as e:
                    logger.error(f"Error writing to wallet file (attempt {attempt + 1}/{max_retries}): {e}")
                    if attempt == max_retries - 1:  # Last attempt
                        raise
                    # Try fallback location on failure
                    WALLET_CSV_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'user_wallets_{attempt + 1}.csv')
                    logger.info(f"Trying fallback location: {WALLET_CSV_FILE}")
