...
async def handle_show_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if existing_wallet:
            # First, acknowledge the callback query immediately
            await query.answer()
            ...
            message = (
                ...
                "✅ Use this to import into Phantom"
            )
            ...
            try:
                ...
                logger.error(f"Error deleting private key message: {str(e)}")
        else:
        ...
        try:
            ...
            logger.error("Failed to send error message for expired query")
...
async def handle_new_wallet_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        message = (
            ...
            "1. Your current wallet will be archived\n"
            "2. You will lose access to the old private key\n"
            "3. Make sure you have saved your current private key\n"
            "4. Any funds should be transferred before proceeding\n\n"
            "Are you sure you want to continue?"
        )
        
        keyboard = [
            [
                InlineKeyboardButton("✅ Yes, Generate New", callback_data="generate_new_confirmed"),
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if query.data.startswith("show_key_"):
            ...
            # Archive old wallet and generate new one
        ...
        await query.answer("Error processing button press", show_alert=True)
...
async def store_new_user(user_id: int, username: str = None, first_name: str = None, last_name: str = None):
    """Store new user with proper error handling and file permissions."""
    try:
        ...
        if not os.path.exists(csv_file):
            try:
                ...
                logger.error(f"Error creating user_data.csv: {str(e)}")
                return False
        ...
        try:
            ...
            logger.error(f"Error reading user_data.csv: {str(e)}")
            return False
        ...
        try:
            ...
            logger.error(f"Error writing to user_data.csv: {str(e)}")
            return False
